...
with st.sidebar:
    st.title("⚙️ Controle")
    if st.button("🔄 Sincronizar Notion", width="stretch"):
...
with tab_fpa:
    ...
    if df_fpa.empty:
        ...
        st.dataframe(df_cascata.style.format("R$ {:,.2f}"), width="stretch")
        ...
        st.dataframe(pivot_fpa.style.format("R$ {:,.2f}"), width="stretch")
